...
let rec eval_memorize e1 env oldenv =
  match e1 with
  | TempList   (e1,e2) -> (e1, (eval_expr oldenv e2)) :: env
  | TempListAdd (e1,e2,e3) -> 
  	let newenv = (eval_memorize e3 env oldenv)
	in
	(e1, (eval_expr oldenv e2)) :: newenv
...
let rec eval_command env c =
  ...
  | AndLet (e1,e2,e3) ->
	...
	(let (_,y,_) = (eval_command env e3) in y),
	((eval_expr env e2) :: (let (_,_,y) = eval_command env e3 in y)) )
  | InEval (e1,e2) ->
    (
	let oldenv = env in
	let newenv = eval_memorize(e1, env, oldenv) in
	(["-"], oldenv, (eval_command newenv e2)) ) 
