...
let rec infer_expr tenv exp =
  ...
  | ELetRec (e1,e2,e3,e4) ->
	 ...
	 let newtenv1 = [(e1,(TyFun(newvar1,newvar2))); (e2, newvar2)] @ tenv in
...
let rec infer_cmd tenv cmd =
  ...
  | CExp e1 ->
	...
	ty_subst ans (fst(infer)))
  | CDecl (e1,e2) ->
    (let infer = infer_expr tenv e2 in
	 let ans = unify (snd(infer)) in
	 ty_subst ans (fst(infer)))
  | CRecDecl (e1,e2,e3) ->
    (let infer = infer_expr tenv e3 in
	 let ans = unify (snd(infer)) in
     ty_subst ans (fst(infer))) 
...
let rec eval_command env c =
  try 
  (let type_result = infer_cmd [] c in
  match c with
  ...
  with  TyError -> ("Error", env, VError("Unmatched type"))
