...
let rec eval_command env tenv c =
  ...
  match c with
  | CExp e -> (print_type(fst(type_result));("-", env, tenv, eval_expr env e))
  | CDecl (e1,e2) -> (print_type(fst(type_result));(e1, (extend e1 (eval_expr env e2) env), (union [(e1, (fst(type_result)))] tenv), eval_expr env e2))
  | CRecDecl (f, x, e) ->
  	let oenv = ref [] in
