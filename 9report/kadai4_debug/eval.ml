...
let rec eval_expr env e =
  ...
  | EVar x ->
    (try
       let Thunk (e_thunk, env_thunk) = Printf.printf "(%s)" x;lookup x env in
	    Printf.printf "{%s}" x;
	   eval_expr (env_thunk) (e_thunk)
     with
     | Unbound ->  Printf.printf "[%s]" x; raise EvalErr)
  | EAdd (e1,e2) ->
  ...
  | EApp (e1, e2) ->
    (match (eval_expr env e1) with
	 ...
	 | VRecFun (f,x,expr1,env_thunk) ->
	   let thunk1 = Thunk (ELecFun (f,x,expr1),env_thunk) in
	   let hoge = (extend "n" (Thunk(EConstInt 1, env)) env ) in
	   let thunk2 = Thunk (e2,env) in
	   Printf.printf "%s" x;
	   eval_expr (extend x thunk2 (extend f thunk1 env_thunk)) expr1)
  | ELecFun (f,x,exp) ->
  let hoge = (extend "n" (Thunk(EConstInt 1, env)) env ) in
  	  VRecFun(f,x,exp,(extend f (Thunk(ELecFun (f,x,exp),env)) env))
  | ELetRec (f, x, e1, e2) ->
  	  let hoge = (extend "n" (Thunk(EConstInt 1, env)) env ) in
  	  let newenv = extend f (Thunk(ELecFun(f,x,e1), env)) env
	  in
	  eval_expr newenv e2
  | EPair (e1,e2) ->
...
let rec eval_command env tenv c =
  ...
  | CRecDecl (f, x, e) ->
  	let newenv = extend f (Thunk(ELecFun(f,x,e),env)) env
  	in
