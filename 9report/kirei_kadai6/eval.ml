...
let rec eval_command env tenv c =
  ...
  match c with
  | CExp e -> Printf.printf "- : ";(print_type(fst(type_result));Printf.printf "%s" " ";("-", env, tenv, eval_expr env e))
  | CDecl (e1,e2) -> Printf.printf "val %s : " e1; (print_type(fst(type_result));Printf.printf "%s" " ";(e1, (extend e1 (eval_expr env e2) env), (snd(type_result)), eval_expr env e2))
  | CRecDecl (f, x, e) ->
  	let oenv = ref [] in
