...
let rec value_approximate func board n =
  if n <= 0 then
    func board (!myiro)
  else
	if n mod 2 = 0 then 
	...
	let rec temp_approximate func board_list ans =
      ...
      | [] -> ans
      | x :: rest ->
        let v = value_approximate func x (n-1) in
        if ans < v then temp_approximate func rest v
                   else temp_approximate func rest ans
    in
    temp_approximate func b_list (-10000)
    )
    with Nomove -> value_approximate func board (n-2))
	else (*奇数の時、minを取っている*)
	(try(
...
let rec decide move_l func mycolor board n acc_move acc_v =
  ...
  | [] -> acc_move
  | x :: rest ->
    let v = value_approximate func (doMove_copy board (Mv(fst(x),snd(x))) mycolor) (n-1) in
