...
let sort_move move_l func board mycolor =
  let rec generate_pair move_l pair_l =
    match move_l with
	| [] -> pair_l
	| x :: rest -> 
	  let v = value_approximate func (doMove_copy board (Mv(fst(x),snd(x))) mycolor) 1 (-10000) in
...
let rec decide move_l func mycolor board n acc_move acc_v =
  let sorted_move = sort_move move_l func board mycolor in
  match sorted_move with
  | [] -> acc_move
  | x :: rest ->
    let v = value_approximate func (doMove_copy board (Mv(fst(x),snd(x))) mycolor) (n-1) (-10000) in
  ...
  let ms = valid_moves board color in
    if ms = [] then
      ...
      if (!count_num) < 12 then
		...
		approximate board count color 6
