...
let create_hash hash filename =
  ...
  let cat_sub () =
    ...
    Hashtbl.add hash ((Int64.of_string black_bit),(Int64.of_string white_bit)) (move1,move2);
    done
  in
...
let play board color =
  ...
  if (!count_num) = 1 then		  
	...
	create_hash my_hash "joseki17.txt";
	create_hash my_hash "joseki18.txt";
	create_hash my_hash "joseki19.txt";
	create_hash my_hash "joseki20.txt";
	);
  let ms = valid_moves board color in
    if ms = [] then
	  ...
	  (*序盤4手読み*)
        ...
        let d = judge_edge2 ms in
        if d = (0,0) then
          let (Mv(a,b)) = pre_approximate board banmen_eval_middle color 6 in
          if a = 0 then
            approximate board banmen_eval_middle color 4
          else Mv(a,b)
          else Mv ((fst(d)),(snd(d)))
	  else if (!count_num) < 33 then
		approximate board count color 12 
