...
let create_hash my_hash filename =
  ...
  let ms = valid_moves board color in
    if ms = [] then
	  ...
	  if (!count_num) < 2 then
        ...
        (Mv (i,j))
      else if (!count_num) = 2 then
        let my_hash = Hashtbl.create 123456 in
        create_hash my_hash "joseki4.txt";
        let b = generate_bit_from_board board in
		print_int (Int64.to_int (fst(b)));
		print_newline ();
		print_string "a";
		print_newline ();
        let (a,b) = Hashtbl.find my_hash ((fst(b)),(snd(b))) in
        Mv (a,b)
	  else if (!count_num) < 16 then 
		try(
		let my_hash = Hashtbl.create 123456 in
		create_hash my_hash "data.txt";
		let b = generate_bit_from_board board in
  		let (a,b) = Hashtbl.find my_hash ((fst(b)),(snd(b))) in
		raise Nomove
		(*Mv (a,b)*)
        ...
        if d = (0,0) then
          approximate board banmen_eval_first color 2
        else Mv ((fst(d)),(snd(d))))
	  else if (!count_num) < 25 then
	    let d = judge_edge2 ms in
		if d = (0,0) then 
          approximate board banmen_eval_middle color 2
        else Mv ((fst(d)),(snd(d)))
